...
def plot_convergence(*args, true_maximum=None, yscale=None, ax=None):
    ...
    true_maximum : float, optional
        The true maximum value of the function, if known.
    ...
    ax.set_ylabel(r"$max$ rate after $n$ calls")
    ax.grid()
    ...
    for index, (arg, color) in enumerate(zip(args, colors)):
        ...
        if isinstance(arg, OptimizeResult):
            ...
            maxs = [np.max(opt_res.func_vals[: i + 1]) for i in range(n_calls)]
            ax.plot(
                range(1, n_calls + 1),
                maxs,
                c=color,
    ...
    if true_maximum:
        ax.axhline(true_maximum, linestyle="--", color="r", lw=1, label="True maximum")

    if true_maximum or label:
        ax.legend(loc="best")
    ...
    for i in range(n_dims):
        for j in range(n_dims):
            if i == j:
                ...
                ax_ = axes[i, j]
                ax_.plot(xi, yi)
                ax_.axvline(minimum[index], linestyle="--", color="r", lw=2)
